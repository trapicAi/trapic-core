...
export class MariaDbAdapter implements DbAdapter {
  ...
  async insertTrace(trace: TraceInsert): Promise<{ id: string } | null> {
    const id = randomUUID();
    await this.pool.execute(
      `INSERT INTO traces (id, content, context, author, tags, confidence) VALUES (?, ?, ?, ?, ?, ?)`,
      [id, trace.content, trace.context ?? null, trace.author, JSON.stringify(trace.tags), trace.confidence]
    );
    return { id };
  }
